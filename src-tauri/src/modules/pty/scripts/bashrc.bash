# opincode-shell-integration (bashrc)
#
...
if [ -z "$__OPINCODE_HOOKS_LOADED" ]; then
  ...
  _opincode_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_opincode_urlencode "$PWD")"
    if [ -z "$__OPINCODE_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __OPINCODE_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_opincode_precmd:"*) ;;
    *) PROMPT_COMMAND="_opincode_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
