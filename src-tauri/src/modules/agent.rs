...
const OWNED_MARKERS: [&str; 3] = ["notify;OpinCode;", "opincode;notify", "opincode;notify"];

// Gated on OPINCODE_TERMINAL; no-op outside OpinCode. Returns the sequence via
// `terminalSequence` because hooks lost /dev/tty access in v2.1.139.
fn hook_cmd(event: &str) -> String {
    format!(
        r#"[ -n "$OPINCODE_TERMINAL" ] && printf '{{"terminalSequence":"\\u001b]777;notify;OpinCode;{event}\\u0007"}}' || true"#
    )
}
...
pub fn agent_enable_claude_hooks() -> Result<(), String> {
    ...
    let tmp = path.with_extension("json.opincode-tmp");
...
pub fn agent_claude_hooks_status() -> bool {
    ...
    HOOK_EVENTS
        .iter()
        .all(|(_, m)| content.contains(&format!("notify;OpinCode;{m}")))
}
...
mod tests {
    ...
    fn adds_all_event_hooks_to_empty_config() {
        ...
        assert!(command(&out, "Notification", 0).contains("notify;OpinCode;attention"));
        assert!(command(&out, "Stop", 0).contains("notify;OpinCode;finished"));
        assert!(command(&out, "UserPromptSubmit", 0).contains("notify;OpinCode;working"));
        ...
        let legacy = json!({
            "hooks": {
                "Notification": [
                    { "hooks": [ {
                        "type": "command",
                        "command": "[ -n \"$OPINCODE_TERMINAL\" ] && printf '\\033]777;opincode;notify\\033\\\\' > /dev/tty || true"
                    } ] }
                ]
            }
        });
