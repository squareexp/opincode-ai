...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S opincode-bin";
    case "debian":
      return `sudo apt install ./OpinCode_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./OpinCode-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `OpinCode v${manual.version} is available`
                  : `OpinCode v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart OpinCode to finish installing."
              : downloading
                ? progress !== null
