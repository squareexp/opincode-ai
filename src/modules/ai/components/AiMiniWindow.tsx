import { InfoCircle, ArrowDown2, CloseCircle, Add, Trash, Filter, Code } from 'iconsax-react';
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-1">
        ...
        <Button
          ...
          title="Close (Esc)"
        >
          <CloseCircle variant="Linear" size={11}  color="currentColor"/>
        </Button>
      </div>
    </div>
  );
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="min-w-56">
        ...
        >
          <Add variant="Linear" size={12} color="currentColor" className="shrink-0" />
          New session
        </DropdownMenuItem>
...
function SessionRow({
  ...
  return (
    ...
    >
      ...
      >
        <Trash variant="Linear" size={11} color="currentColor" className="shrink-0" />
      </button>
    </DropdownMenuItem>
  );
}
...
function EmptyState({ onPick }: { onPick: (text: string) => void }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 px-8 py-10 text-center">
      <img src="/logo.png" alt="OpinCode" className="size-14 opacity-90" />
      <div className="space-y-1.5">
        <p className="text-[14px] font-semibold tracking-tight">
          Ask OpinCode anything
        </p>
        <p className="max-w-[18rem] text-[11.5px] leading-relaxed text-muted-foreground">
          OpinCode sees the active terminal — cwd, recent commands, and output.
        </p>
      </div>
        ...
        {SUGGESTIONS.map((s) => (
          ...
          >
            <div className="flex size-7 shrink-0 items-center justify-center rounded-md bg-muted/70 text-muted-foreground transition-colors group-hover:bg-foreground/5 group-hover:text-foreground">
              <s.icon variant="Linear" size={13} color="currentColor" className="shrink-0" />
            </div>
