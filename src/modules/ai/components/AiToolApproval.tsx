import { Edit2, CloseCircle, TickCircle, DocumentCode, FolderAdd, Code, Category } from 'iconsax-react';
...
const TOOL_META: Record<string, { label: string; icon: any }> =
  {
    write_file: { label: "Write file", icon: DocumentCode },
    edit: { label: "Edit file", icon: Edit2 },
    multi_edit: { label: "Edit file (batch)", icon: Edit2 },
    create_directory: { label: "Create directory", icon: FolderAdd },
    bash_run: { label: "Run shell command", icon: Code },
    bash_background: { label: "Spawn background process", icon: Code },
  };
    ...
    <div className="rounded-lg border border-border bg-card shadow-sm">
      ...
      <div className="flex items-center justify-end gap-1.5 border-t border-border/60 px-3 py-2">
        ...
        <Button
          ...
          className="h-7 gap-1.5 text-[11px]"
        >
          <TickCircle variant="Linear" size={12} color="currentColor" />
          Approve
        </Button>
      </div>
    </div>
