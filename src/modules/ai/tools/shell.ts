...
export function buildShellTools(ctx: ToolContext) {
  return {
    ...
    bash_background: tool({
      description:
        "Spawn a long-running background process (e.g. `bun dev`, `cargo watch`, log tailers). Returns a handle; use `bash_logs` to read its output and `bash_kill` to stop it. Output is captured into a 4MB ring buffer. Asks for user approval.",
      inputSchema: z.object({
        command: z.string(),
