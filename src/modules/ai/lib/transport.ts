...
const OPINCODE_MD_MAX_BYTES = 32 * 1024;
type MemoryCacheEntry = { content: string | null; mtime: number };
const projectMemoryCache = new Map<string, MemoryCacheEntry>();

async function readOpincodeMd(workspaceRoot: string | null): Promise<string | null> {
  if (!workspaceRoot) return null;
  const path = `${workspaceRoot.replace(/\/$/, "")}/OPINCODE.md`;
  ...
  try {
    ...
    const content =
      r.content.length > OPINCODE_MD_MAX_BYTES
        ? r.content.slice(0, OPINCODE_MD_MAX_BYTES)
        : r.content;
...
export function createContextAwareTransport(deps: Deps) {
  const run = async (options: SendOptions) => {
    ...
    const projectMemory = await readOpincodeMd(live.workspaceRoot);
