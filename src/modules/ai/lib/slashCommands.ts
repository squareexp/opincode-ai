import { SparklesIcon, CheckListIcon, ClaudeIcon } from '@hugeicons/core-free-icons';
...
const INIT_PROMPT = `Scan this workspace and produce OPINCODE.md at the workspace root with:
...
Use grep/glob/list_directory/read_file to explore. Cap OPINCODE.md under 200 lines. Use write_file to create it (will go through normal approval).`;
...
export const OPINCODE_CMD_RE =
  /^<opincode-command\s+name="([a-z0-9-]+)"(?:\s+state="([a-z]+)")?\s*\/>(?:\n+|$)/;

export function wrapWithCommandMarker(prompt: string, name: string): string {
  return `<opincode-command name="${name}" />\n\n${prompt}`;
}
