...
// ---------------------------------------------------------------------------
// Serialization helpers for layout persistence
// ---------------------------------------------------------------------------

export function serializeTabLayout(
  tabs: Tab[],
  activeId: number,
  nextId: number,
): PersistedTabLayout {
  const serializedTabs: PersistedTerminalTab[] = tabs
  ...
  return { version: 1, activeId, nextId, tabs: serializedTabs };
}

export function deserializeTabLayout(
  ...
  // Validate there is at least one tab
  if (tabs.length === 0) {
    const tabId = layout.nextId;
    const leafId = layout.nextId + 1;
    return {
      tabs: [
        {
          id: tabId,
      ...
      activeId: tabId,
      nextId: layout.nextId + 2,
    };
  }
  // Make sure activeId refers to an existing tab
  const activeId = tabs.some((t) => t.id === layout.activeId)
    ? layout.activeId
    : tabs[0].id;
  return { tabs, activeId, nextId: layout.nextId };
}
...
export function useTabs(
  ...
  /** Pre-loaded persisted layout; when provided, overrides initial. */
  persistedLayout?: PersistedTabLayout | null,
) {
  const [tabs, setTabs] = useState<Tab[]>(() => {
    if (persistedLayout && persistedLayout.tabs.length > 0) {
      return deserializeTabLayout(persistedLayout).tabs;
    }
    const tabId = 1;
    const leafId = 2;
    return [
  ...
  const [activeId, setActiveId] = useState(() => {
    if (persistedLayout && persistedLayout.tabs.length > 0) {
      return deserializeTabLayout(persistedLayout).activeId;
    }
    return 1;
  });
  const nextIdRef = useRef(
    persistedLayout && persistedLayout.tabs.length > 0
      ? deserializeTabLayout(persistedLayout).nextId
      : 3,
  );
  ...
  // Auto-save layout whenever tabs or activeId changes.
  useEffect(() => {
    const layout = serializeTabLayout(tabs, activeId, nextIdRef.current);
