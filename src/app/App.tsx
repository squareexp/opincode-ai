...
import { MAX_PANES_PER_TAB, usePersistedTabs, useWorkspaceCwd } from "@/modules/tabs";
import {
  disposeSession,
  findLeafCwd,
  hasLeaf,
...
const SIDEBAR_WIDTH_STORAGE_KEY = "opincode.sidebar.width";
const SIDEBAR_VIEW_STORAGE_KEY = "opincode.sidebar.view";
...
export default function App() {
  const {
    ...
    closePaneByLeaf,
    resetWorkspace,
  } = usePersistedTabs(getLaunchDir() ? { cwd: getLaunchDir() } : undefined);
      ...
      (event) => {
        ...
        void (async () => {
          try {
            ...
            if (!parsed.ok) {
              console.warn("[opincode] theme not applied:", parsed.error);
              return;
            }
            await saveCustomTheme(parsed.theme);
          } catch (e) {
            console.warn("[opincode] theme ingest failed:", e);
          }
        })();
      },
    );
  ...
  const handleAttachFileToAgent = useCallback(
    (path: string) => {
      ...
      window.dispatchEvent(
        new CustomEvent<string>("opincode:ai-attach-file", { detail: path }),
      );
      openPanel();
      focusInput(null);
    },
