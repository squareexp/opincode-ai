...
const TOOL_META: Record<string, { label: string; icon: any }> = {
  read_file: { label: "Read", icon: DocumentText },
  list_directory: { label: "List", icon: FolderOpen },
  write_file: { label: "Write", icon: DocumentCode },
  create_directory: { label: "Create dir", icon: FolderAdd },
  edit: { label: "Edit", icon: Edit2 },
  multi_edit: { label: "Edit", icon: Edit2 },
  bash_run: { label: "Run", icon: Code },
  bash_background: { label: "Spawn", icon: Code },
  bash_logs: { label: "Logs", icon: Code },
  bash_list: { label: "Jobs", icon: Code },
  bash_kill: { label: "Kill", icon: Code },
  grep: { label: "Search", icon: SearchNormal1 },
  glob: { label: "Glob", icon: Folder2 },
  suggest_command: { label: "Suggest", icon: Magicpen },
  open_preview: { label: "Preview", icon: Eye },
  run_subagent: { label: "Subagent", icon: Cpu },
  todo_write: { label: "Todos", icon: TickSquare },
};
...
const ToolImpl = ({
  ...
  return (
    ...
    >
      ...
      {hasDetails && (
        <CollapsibleContent
          className={cn("opincode-collapsible-content")}
        >
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex items-stretch gap-1.5 rounded bg-muted/40 overflow-hidden">
        ...
        >
          {inserted ? (
            <Code1 variant="Linear" size={12} color="currentColor" className="shrink-0" />
          ) : (
            <ArrowRight2 variant="Linear" size={12} color="currentColor" className="shrink-0" />
          )}
