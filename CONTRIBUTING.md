# Contributing to OpinCode

OpinCode is a solo-maintained project with a strong product direction. Contributions are welcome, but **alignment matters more than volume**.
...
Prereqs: Rust (stable), Bun (stable), plus your platform's [Tauri prerequisites](https://tauri.app/start/prerequisites/).

## Where to discuss

Discord: [ajmalleonard OS](https://discord.gg/tyveTUyEp7)
...
OpinCode positions itself as **lightweight, fast, production-grade**. Every PR is reviewed against:

- `bun x tsc --noEmit` clean
- `cargo clippy` clean, `cargo fmt` applied
- `bun test` and `cargo test` pass
...
The most common way a PR breaks OpinCode is a **local fix with global blast radius**: the diff solves one reported case, reads fine, passes type-check and clippy, and silently breaks the same subsystem in every other case. Review alone does not catch these. A test does.
...
- OpinCode is not trying to be a full IDE replacement (VS Code, Cursor, Zed).
